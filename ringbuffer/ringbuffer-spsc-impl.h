...
namespace PoC::LockFree {
    ...
    private:
        ...
        std::atomic<size_t> m_write_ptr; // Points to the NEXT available position to write, i.e., the tail end of the queue
        std::atomic<size_t> m_read_ptr;  // Points to the NEXT available position to read, i.e., the head end of the queue
    public:
        // we want to distinguish between buffer empty (tail == head) and buffer
        // full (tail + 1 == head), so we need the allocate capacity+1
        explicit RingBufferSPSC(const size_t capacity) : m_capacity(
                capacity + 1), m_buffer(capacity + 1), m_write_ptr(0),
                                                         m_read_ptr(0) {}
        ...
        // T&& is NOT a universal reference because T is already a fixed type (not deduced).
        // U&& IS a universal reference because U is deduced dynamically.
        template<typename U>
        // Conversion (std::convertible_to<U, T>) creates a new object, so no reference is needed.
        // Assignment (std::assignable_from<T&, U>) modifies an existing object, so T& is required.
        ...
        bool dequeue(T &item) {
            ...
            m_read_ptr.store((head + 1) % m_capacity,
