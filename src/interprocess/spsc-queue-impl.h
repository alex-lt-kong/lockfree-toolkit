...
namespace RingBuffer::Interprocess {
  ...
  public:
    ...
    bool dequeue_impl(std::string &buffer) const {
      ...
      // for tail_atomic.load(), std::memory_order_relaxed works on x86 but breaks on ARM64
  ...
  template<typename U> requires std::assignable_from<std::string &, U>
  bool SpscQueue::enqueue_impl(U &&msg_bytes) {
    ...
    const std::atomic_ref head_atomic(*head_ptr);
    const std::atomic_ref tail_atomic(*tail_ptr);
    // for head_atomic.load(), std::memory_order_relaxed works on x86 but breaks on ARM64
    const int head = head_atomic.load(std::memory_order_acquire);
    const int tail = tail_atomic.load(std::memory_order_acquire);

    const int used = get_used_bytes(head, tail);
    if (const int free = m_queue_size - used; free < element_length * 2) {
      //  std::cout << "head: " << head << ", tail: " << tail << ", used: " << used << "\n";
      return false;
    }
