...
namespace RingBuffer::Intraprocess {
    template<typename T>
    class SpscQueue : public RingBuffer::IRingBuffer<SpscQueue<T>, T> {
    private:
        /*
    ...
    public:
        ...
        // reference is needed (i.e., we have T, not T&). Assignment (std::assignable_from<T&, U>) modifies an
        // existing object, so T& is required.
        ...
        int head_impl() const { return 0; }

        int tail_impl() const { return 0; }

        void init_impl() {};

        // int get_usedBytes() const { return 0; }
    };
} // namespace LockFree
