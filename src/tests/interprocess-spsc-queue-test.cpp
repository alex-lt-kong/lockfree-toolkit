...
TEST(InterprocessSpscQueue, ConcurrentProduceAndConsume) {
  ...
  std::thread thread_consumer(common_consumer, qsz_bytes, iter_size, dummy_payloads, "ConcurrentProduceAndConsume");
  // Seems the setup of a shared memory queue is not atomic, need to wait to
  // avoid race condition
  std::this_thread::sleep_for(std::chrono::seconds(1));
  std::thread thread_producer(common_producer, qsz_bytes, iter_size, dummy_payloads, "ConcurrentProduceAndConsume");
  thread_producer.join();
  thread_consumer.join();
}


TEST(InterprocessSpscQueue, ConcurrentProduceAndConsumeOnlyEmptyMessagesAndTightQueue) {
  std::vector<std::string> dummy_payloads = {
    ""
  };
  // TODO: qsz_bytes = sizeof(int) * 2 should work by rights, but it doesnt for now...
  constexpr std::size_t qsz_bytes = sizeof(int) * 2 + 1;
  constexpr std::size_t iter_size = INT32_MAX - 111;
...
TEST(InterprocessSpscQueue, ConcurrentProduceAndConsumeOnlyNullTerminatedCharMessagesAndTightQueue) {
  std::vector<std::string> dummy_payloads = {
    std::string("\x00", 1), std::string("\x00\x00", 2), std::string("\x00\x00\x00", 3)
  };
